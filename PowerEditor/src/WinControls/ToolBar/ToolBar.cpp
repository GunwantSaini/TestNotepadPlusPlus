...
{
	NppXml::Node toolButtons = NppXml::firstChildElement(toolButtonsDocRoot, "NotepadPlus");
	if (toolButtons)
	{
		...
		if (toolButtons)
		{
			...
			NppXml::Node standardToolButtons = NppXml::firstChildElement(toolButtons, "Standard");
			if (standardToolButtons)
			{
				_toolbarStdButtonsConfArray = std::make_unique<bool[]>(arraySize);

				NppXml::Element stdBtnElement = standardToolButtons->ToElement();
				const char* isHideAll = NppXml::attribute(stdBtnElement, "hideAll");
				...
				else
				{
					...
					for (NppXml::Node childNode = NppXml::firstChildElement(standardToolButtons, "Button");
						childNode;
						childNode = NppXml::nextSiblingElement(childNode, "Button"))
					{
						NppXml::Element element = childNode->ToElement();
						int cmdID = NppXml::intAttribute(element, "id", -1);

						int index = NppXml::intAttribute(element, "index", -1);

						const char* isHide = NppXml::attribute(element, "hide");
			...
			NppXml::Node pluginToolButtons = NppXml::firstChildElement(toolButtons, "Plugin");
			if (pluginToolButtons)
			{
				NppXml::Element pluginBtnElement = pluginToolButtons->ToElement();
				const char* isHideAll = NppXml::attribute(pluginBtnElement, "hideAll");
				if (isHideAll && (std::strcmp(isHideAll, "yes") == 0))
				{
				...
				for (NppXml::Node childNode = NppXml::firstChildElement(pluginToolButtons, "Button");
					childNode;
					childNode = NppXml::nextSiblingElement(childNode, "Button"))
				{
					bool doShow = true;
