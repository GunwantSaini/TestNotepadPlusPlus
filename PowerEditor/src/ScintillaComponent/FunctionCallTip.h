...
public:
	...
	void setLanguageXML(NppXml::Element pXmlKeyword); //set calltip keyword node
...
private:
	...
	NppXml::Element _pXmlKeyword = nullptr; //current keyword node (first one)

	intptr_t _curPos = 0;					//cursor position
	intptr_t _startPos = 0;					//display start position

	NppXml::Element _curFunction = nullptr; //current function element
