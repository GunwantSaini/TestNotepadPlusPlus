...
bool AutoCompletion::setLanguage(LangType language)
{
	...
	NppXml::Node pAutoNode = nullptr;
	if (_funcCompletionActive)
	{
		_funcCompletionActive = false; //safety
		NppXml::Element pNode = NppXml::firstChildElement(_pXmlFile, "NotepadPlus");
		if (!pNode)
			return false;

		pAutoNode = pNode = NppXml::firstChildElement(pNode, "AutoComplete");
		if (!pNode)
			return false;

		pNode = NppXml::firstChildElement(pNode, "KeyWord");
		if (!pNode)
			return false;
	...
	if (_funcCompletionActive) //try setting up environment
	{
		...
		NppXml::Element pElem = NppXml::firstChildElement(pAutoNode, "Environment");
		if (pElem)
		{
			const char* val = nullptr;
			val = NppXml::attribute(pElem, "ignoreCase");
			if (val && std::strcmp(val, "no") == 0)
			{
				_ignoreCase = false;
				_funcCalltip._ignoreCase = false;
			}
			val = NppXml::attribute(pElem, "startFunc");
			if (val && val[0])
				_funcCalltip._start = val[0];
			val = NppXml::attribute(pElem, "stopFunc");
			if (val && val[0])
				_funcCalltip._stop = val[0];
			val = NppXml::attribute(pElem, "paramSeparator");
			if (val && val[0])
				_funcCalltip._param = val[0];
			val = NppXml::attribute(pElem, "terminal");
			if (val && val[0])
				_funcCalltip._terminal = val[0];
			val = NppXml::attribute(pElem, "additionalWordChar");
			if (val && val[0])
	...
	if (_funcCompletionActive)
	{
		...
		for (NppXml::Element funcNode = _pXmlKeyword;
			funcNode;
