...
use windows::Win32::UI::WindowsAndMessaging::{
    CreateWindowExW, DefWindowProcW, EndDialog, GetDlgItem, GetWindowTextW, HMENU,
    WINDOW_EX_STYLE, WINDOW_STYLE, WM_CLOSE, WM_COMMAND, WM_INITDIALOG, WS_BORDER, WS_CAPTION,
        ...
        WM_COMMAND => {
            ...
            match control_id {
                IDC_FIND_NEXT => {
                    ...
                    if len > 0 {
                        let search_text = String::from_utf16_lossy(&buffer[..len as usize]);
                        log::info!("Searching for: {}", search_text);

                        // TODO: Perform actual search
                        // For now, just show message
                        let msg_text: Vec<u16> = format!("Search for: {}", search_text)
                            .encode_utf16()
